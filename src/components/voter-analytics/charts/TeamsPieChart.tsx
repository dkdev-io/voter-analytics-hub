...
export const TeamsPieChart: React.FC<TeamsPieChartProps> = ({ data, total }) => {
  // Filter out zero values and ensure colors
  ...
  const actualTotal = calculatedTotal > 0 ? calculatedTotal : 1;
  ...
  const renderLegend = (props: any) => {
    ...
    if (!payload || payload.length === 0) {
      return <div className="text-xs text-center mt-2">No team data available</div>;
    }
        ...
        {payload.map((entry: any, index: number) => (
          <li key={`legend-item-${index}`} className="flex items-center mb-1">
            ...
            <span className="whitespace-nowrap text-xs">
              {entry.value} - {entry.payload.value.toLocaleString()} ({((entry.payload.value / actualTotal) * 100).toFixed(1)}%)
            </span>
          </li>
        ))}
      </ul>
  ...
  if (filteredData.length === 0) {
    return (
      <div className="h-96 rounded-lg border border-gray-200 flex flex-col">
        <h3 className="text-sm font-bold p-4 text-center border-b">Team Performance</h3>
